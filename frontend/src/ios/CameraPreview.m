...
- (void) startCamera:(CDVInvokedUrlCommand*)command {
  ...
  if (command.arguments.count > 3) {
    ...
    NSString *cameraOverlayImageBase64 = command.arguments[12];
    ...
    if (toBack) {
      ...
      NSURL *url = [NSURL URLWithString:cameraOverlayImageBase64];
      NSData *imageData = [NSData dataWithContentsOfURL:url];
      UIImage *image = [UIImage imageWithData:imageData];

      CGFloat overlaySideLength = 0.0;
      CGFloat x = 0.0;
      CGFloat y = 0.0;
      if (self.cameraRenderController.view.bounds.size.width > self.cameraRenderController.view.bounds.size.height) {
          overlaySideLength = self.cameraRenderController.view.bounds.size.height;
          CGFloat heightDiff = self.cameraRenderController.view.bounds.size.width - self.cameraRenderController.view.bounds.size.height;
          x = heightDiff / 2;
      } else {
          overlaySideLength = self.cameraRenderController.view.bounds.size.width;
          CGFloat heightDiff = self.cameraRenderController.view.bounds.size.height - self.cameraRenderController.view.bounds.size.width;
          y = heightDiff / 2;
      }

      UIImageView *imageView = [[UIImageView alloc] initWithFrame:CGRectMake(x, y, overlaySideLength, overlaySideLength)];
      [imageView setImage:image];
      imageView.alpha = 0.5;
      [self.cameraRenderController.view addSubview:imageView];
    }
